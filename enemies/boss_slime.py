...
from enemies.boss_gunner_slime import BossGunnerSlime # 🚩 신규 거너 임포트
...
class BossSlime(Slime):
    ...
    def update(self, target_player_world_x, target_player_world_y, game_entities_lists):
        ...
        # --- 패턴 0: 각성 로직 (3번째 보스부터, 체력 20% 이하) ---
        if self.boss_index >= config.BOSS_AWAKEN_COUNT and not self.is_phase2:
            if self.hp < (self.max_hp * 0.2):
                print("!!! 보스 각성: 진정한 힘을 개방합니다 !!!")
                self.is_phase2 = True
                self.stop_timer = config.FPS * 1.5 # 1.5초 정지
                self.hp = self.max_hp * 0.7        # 체력 70% 회복
                self.speed *= config.BOSS_PHASE2_SPEED_MULT # 속도 증가
        ...
        if self.is_phase2:
            regen_rate *= config.BOSS_PHASE2_REGEN_MULT
        ...
        # --- 패턴 2: 정예 거너 소환 (각성 전용) ---
        # 🚩 ShooterSlime 대신 BossGunnerSlime을 소환하여 경험치/킬수 제외
        if self.is_phase2:
            ...
            if self.shooter_summon_timer <= 0:
                ...
                if slimes is not None:
                    for _ in range(5):
                        ...
                        slimes.append(BossGunnerSlime(sx, sy, self.initial_spawn_hp_for_minions))

        # --- 패턴 3: 동서남북 거대 탄환 (각성 전용) ---
        if self.is_phase2:
            ...
            if self.big_bullet_timer <= 0:
                ...
                if bullets is not None:
                    ...
                    big_b.size = config.SLIME_BULLET_SIZE * 3
                    big_b.lifespan = config.FPS * 5
